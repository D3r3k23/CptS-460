...
int kfork(char* filename)
{
    ...
    for (int i = 2048; i < 4096; i++) // zero out high 2048 entries
        p->pgdir[i] = 0;

    // Assume 2MB Umode area
    ...
    static char* MY_NAME = "My name is Derek";

    p->kstack[SSIZE - 1] = VA(0);

    u32 upa = p->pgdir[2048] & 0xFFFF0000;
    u32 usp = upa + 0x100000 - 128;

    strcpy((char*)usp, MY_NAME);

    p->usp = (int*)VA(0x100000 - 128);

    // Pass name string char* as first argument to main
